...
from src.models.ctr_ensemble_impressions import evaluate_ctr_ensemble
...
def main() -> int:
    print("=" * 116)
    print("SEO 90-DAY CTR ENSEMBLE IMPRESSION FORECAST - DEVELOPMENT EVALUATION")
    print("=" * 116)
    print("[INFO] Uses passed 90-day calibrated clicks.")
    print("[INFO] Final holdout is NOT used for model/ensemble/bias-correction selection.")
    print("[INFO] Ensemble and bias correction are selected from pre-cutoff OOF folds only.")
    ...
    print()
    print(f"[INFO] Passed 90-day calibrated click total: {click_total:.2f}")

    result = evaluate_ctr_ensemble(
    ...
    result.base_ranking.to_csv(
        OUTPUT_DIR / "seo_ml_90d_ctr_ensemble_base_ranking.csv",
        index=False,
    )
    result.ensemble_ranking.to_csv(
        OUTPUT_DIR / "seo_ml_90d_ctr_ensemble_ranking.csv",
        index=False,
    )
    result.final_predictions.to_csv(
        OUTPUT_DIR / "seo_ml_90d_ctr_ensemble_final_ctrs.csv",
        index=False,
    )
    result.summary.to_csv(
        OUTPUT_DIR / "seo_ml_90d_ctr_ensemble_summary.csv",
        index=False,
    )
    result.daily.to_csv(
        OUTPUT_DIR / "seo_ml_90d_ctr_ensemble_daily.csv",
        index=False,
    )

    print()
    print("PRE-CUTOFF BASE CTR RANKING")
    print(result.base_ranking.to_string(index=False))

    print()
    print("PRE-CUTOFF ENSEMBLE RANKING")
    print(result.ensemble_ranking.to_string(index=False))

    print()
    print("FINAL BASE CTR PREDICTIONS")
    ...
    candidate = result.summary.loc[
        result.summary["Method"].eq("CTREnsembleImpressionsML")
    ].iloc[0]
    ...
    gate.to_csv(
        OUTPUT_DIR / "seo_ml_90d_ctr_ensemble_gate.csv",
        index=False,
    )
